...
const salaryInput = document.getElementById('salaryAdd');

// Получение ссылки на форму
const addTeacherForm = document.getElementById('addModal').querySelector('form');
...
addTeacherForm.addEventListener('submit', function(event) {
    ...
    // Проверка диапазона значения зарплаты
    const salaryValue = parseFloat(salaryInput.value.trim());
    if (salaryValue < 626 || salaryValue > 5000) {
        createToast('error', 'fa-solid fa-circle-exclamation', 'Error', 'Salary must be between 626 and 5000!');
        return;
    }
